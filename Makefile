postgres:
	docker run --name postgres17 --network bank-network -p 5432:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=root -d postgres:17.2-alpine
...
mock:
	mockgen -package mockdb -destination db/mock/store.go github.com/Somraj2929/simple-banking/db/sqlc Store		
